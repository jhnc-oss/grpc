...
bool ValidateMetadataContents(grpc_metadata_batch* metadata) {
  ...
  if (!user_agent || user_agent->as_string_view() != "grpc-c/0.12.0.0 (linux)")
    return false;
  auto* authority = metadata->get_pointer(HttpAuthorityMetadata());
  if (!authority || authority->as_string_view() != "localhost") return false;
  auto* path = metadata->get_pointer(HttpPathMetadata());
  if (!path || path->as_string_view() != "/foo/bar") return false;
  ...
  if (!te || *te != TeMetadata::kTrailers) return false;
  ...
  if (!scheme || *scheme != HttpSchemeMetadata::kHttp) return false;
  auto* method = metadata->get_pointer(HttpMethodMetadata());
  if (!method || *method != HttpMethodMetadata::kPost) return false;
  auto* status_from_wire = metadata->get_pointer(GrpcStatusFromWire());
  if (!status_from_wire || !*status_from_wire) return false;
  return true;
}
