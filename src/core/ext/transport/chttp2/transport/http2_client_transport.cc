...
absl::Status Http2ClientTransport::AckPing(uint64_t opaque_data) {
  ...
  if (ping_manager_->AckPing(opaque_data) &&
      ping_manager_->ImportantPingRequested()) {
      ...
      Loop([self = RefAsSubclass<Http2ClientTransport>()]() {
        return Map(
            ...
            [self](Empty) -> LoopCtl<Empty> {
              ...
              absl::Status status = self->TriggerWriteCycle();
              if (!status.ok()) {
                GRPC_UNUSED absl::Status error = self->HandleError(
                    /*stream_id=*/std::nullopt, ToHttpOkOrConnError(status));
                return Empty{};
...
void Http2ClientTransport::ActOnFlowControlAction(
  ...
  if (action.AnyUpdateImmediately()) {
    ...
    if (!status.ok()) {
      GRPC_HTTP2_CLIENT_DLOG
          << "Http2ClientTransport::ActOnFlowControlAction Failed to "
             "trigger write cycle with status: "
          << status;
      GRPC_UNUSED absl::Status unused =
          HandleError(std::nullopt, ToHttpOkOrConnError(status));
      return;
    }
...
void Http2ClientTransport::SpawnTransportLoops() {
  ...
  if (!status.ok()) {
    GRPC_HTTP2_CLIENT_DLOG
        << "Http2ClientTransport::SpawnTransportLoops Failed to trigger "
           "write cycle: "
        << status;
    GRPC_UNUSED absl::Status result = HandleError(
        /*stream_id=*/std::nullopt, ToHttpOkOrConnError(status));
    return;
  }
