 ...
 protected:
  QueryableDomain(std::string name, InstrumentLabelList label_names,
      ...
      : label_names_(std::move(label_names)),
        ...
        name_(std::move(name)) {
          LOG(INFO) << "QueryableDomain " << name_ << " created with " << map_shards_size_ << " map shards." << " Labels: " << label_names_.DebugString();
        }
 ...
 protected:
  ...
  static const FixedInstrumentLabelList<sizeof...(Label)> MakeLabels(
    ...
    for (size_t i = 0; i < sizeof...(Label); ++i) {
      ...
      LOG(INFO) << Derived::kName << "[" << i << "] " << list[i].label() << " " << int(list[i].index());
      for (size_t j = i + 1; j < sizeof...(Label); ++j) {
